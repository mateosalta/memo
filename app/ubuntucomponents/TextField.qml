...
Ubuntu.ActionItem {
    ...
    */
    ...
    /*!
      ...
      \list
        \li - TextInput.NoWrap - no wrapping will be performed. If the text contains
            insufficient newlines, then implicitWidth will exceed a set width.
        \li - TextInput.WordWrap - wrapping is done on word boundaries only.
            If a word is too long, implicitWidth will exceed a set width.
        \li - TextInput.WrapAnywhere - wrapping is done at any point on a line,
            even if it occurs in the middle of a word.
        \li - TextInput.Wrap - if possible, wrapping occurs at a word boundary;
    ...
    */
    ...
    /*!
      ...
      Note that if there is a validator or inputMask set on the text input and enter/return is pressed,
      this signal will only be emitted if the input follows the inputMask and the validator
    ...
    */
    ...
    Component.onCompleted: {
        ...
        editor.editingFinished.connect(control.editingFinished);
        cursorPosition = 0;
    }
